          ...
          <Card className="border-primary/20 bg-card/40 backdrop-blur-xl">
             <CardHeader>
                <div className="flex items-center gap-3">
                   ...
                   <div>
                      <CardTitle className="font-mono uppercase tracking-tighter italic text-lg leading-none">Net Configuration</CardTitle>
                      <CardDescription className="font-mono text-[10px] opacity-60 uppercase tracking-widest mt-1">Configure your endpoint address</CardDescription>
                   </div>
                </div>
             </CardHeader>
             <CardContent className="space-y-6">
                <div className="space-y-2">
                   <label className="text-[10px] font-mono text-primary/60 uppercase tracking-widest pl-1">IP ADDRESS</label>
                   <Input 
                     value={ip}
                ...
                <div className="space-y-3">
                   <label className="text-[10px] font-mono text-primary/60 uppercase tracking-widest pl-1 italic">SUBNET MASK (CIDR)</label>
                       ...
                       {[32, 30, 24, 16, 8].map(c => (
                         <Button
                           ...
                           className={cn(
                             ...
                             cidr === c ? "bg-primary/20 border-primary/40 text-primary" : "hover:border-primary/30"
                           )}
                           onClick={() => setCidr(c)}
                         >
                            /{c}
                ...
                <div className="mt-8 p-4 bg-primary/5 border-l-2 border-primary rounded-sm space-y-1">
                   <div className="text-[9px] font-mono text-primary uppercase font-bold tracking-[0.2em]">Network Type</div>
                   <p className="font-mono text-xs text-foreground/80 italic">
                      {cidr === 32 ? "Single Host Node" : cidr >= 24 ? "Class C (Small Network)" : cidr >= 16 ? "Class B (Medium-Large)" : "Class A (Enterprise Hub)"}
                   </p>
                </div>
             </CardContent>
          </Card>
           ...
           {result ? (
              ...
              >
                  <Card className="bg-black/60 border-primary/20 p-6 space-y-4">
                     <div className="flex items-center gap-2 text-primary">
                        ...
                        <span className="text-[10px] uppercase font-mono font-bold tracking-widest">Network Geometry</span>
                     </div>
                     <div className="space-y-4">
                        {[
                          ...
                          <div key={item.label} className="flex justify-between items-center border-b border-white/5 pb-2">
                             <div className="text-[9px] font-mono text-white/40 uppercase tracking-widest">{item.label}</div>
                             <div className="text-sm font-mono text-primary italic font-bold tracking-tight">{item.val}</div>
                          </div>
                        ))}
                     </div>
                  </Card>
                  ...
                  <Card className="bg-black/60 border-primary/20 p-6 space-y-4">
                     <div className="flex items-center gap-2 text-primary">
                        ...
                        <span className="text-[10px] uppercase font-mono font-bold tracking-widest">Asset Statistics</span>
                     </div>
                     <div className="flex flex-col items-center justify-center py-4 space-y-2">
                        ...
                        <div className="text-[10px] uppercase tracking-widest text-foreground/40 font-bold">Allocatable Host nodes</div>
                     </div>
                  ...
                  <Card className="md:col-span-2 bg-black border border-primary/20 overflow-hidden relative group">
                     ...
                     <div className="bg-white/5 p-4 border-b border-white/5 flex items-center justify-between">
                        <div className="flex items-center gap-2">
                           ...
                           <span className="text-[10px] uppercase font-mono font-bold tracking-[0.2em] text-primary">Binary Decomposition matrix</span>
                        </div>
                        <Badge variant="outline" className="text-[8px] font-mono border-primary/20 uppercase tracking-widest">Bitset Extraction</Badge>
                     </div>
                     <div className="p-8 space-y-6 font-mono relative">
                        <div className="space-y-4">
                           <div className="space-y-2">
                              <div className="text-[8px] text-white/30 uppercase tracking-[0.3em] font-bold">Network bitset</div>
                              <div className="text-primary tracking-[0.4em] font-black text-xs md:text-lg break-all">
                                 {result.binary.mask}
                              </div>
                           </div>
                           <div className="space-y-2">
                              <div className="text-[8px] text-white/30 uppercase tracking-[0.3em] font-bold">Address bitset</div>
              ...
              <div className="h-full flex items-center justify-center border border-primary/10 border-dashed rounded-xl py-40">
                 <div className="text-center space-y-4">
                    ...
                    <p className="font-mono text-[10px] uppercase tracking-widest text-primary/40 italic">Waiting for valid IP node resolution...</p>
                 </div>
              </div>
           )}
        </div>
