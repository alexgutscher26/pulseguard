      ...
      <Card className="border-primary/20 bg-card/40 backdrop-blur-xl relative overflow-hidden group">
        ...
        <CardHeader>
           <div className="flex items-center gap-3">
              ...
              <div>
                 <CardTitle className="font-mono uppercase tracking-tighter italic text-lg leading-none">DNS Integrity Sentinel</CardTitle>
                 <CardDescription className="font-mono text-[10px] opacity-60 uppercase tracking-widest mt-1">Audit MX, SPF, & DMARC Health Score</CardDescription>
              </div>
           </div>
        </CardHeader>
        <CardContent>
         ...
         {isAuditing && (
           ...
           >
                 ...
                 {[...Array(6)].map((_, i) => (
                    <motion.div 
                      ...
                      animate={{ 
                        ...
                        backgroundColor: activeStep >= i ? ["#22c55e", "#16a34a", "#22c55e"] : "#22c55e33"
                      }}
              ...
              <div className="text-center space-y-2">
                 <GlitchText text={resolveSteps[activeStep]} className="text-primary font-mono text-xl uppercase tracking-tighter italic" />
                 <div className="text-[10px] font-mono text-primary/40 uppercase tracking-[0.3em] font-bold">Query: {domain}</div>
              </div>
           </motion.div>
         )}
           ...
           >
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                 <Card className="md:col-span-1 bg-black border border-primary/20 flex flex-col items-center justify-center py-12 relative overflow-hidden group">
                    ...
                    <div className="text-[10px] font-mono text-primary/60 uppercase tracking-widest absolute top-4">Integrity Level</div>
                    <div className="text-8xl font-black font-mono text-primary italic tracking-tighter group-hover:scale-110 transition-transform">{auditData.grade}</div>
                 </Card>

                 <div className="md:col-span-3 grid grid-cols-3 gap-4">
                    {[
                      { label: "Deliverability", val: `${auditData.score}%`, icon: <Mail className="w-4 h-4 text-primary" /> },
                      { label: "Security Nodes", val: auditData.results.filter(r => r.status === "SECURE").length, icon: <ShieldCheck className="w-4 h-4 text-primary" /> },
                      { label: "Total Records", val: Object.keys(auditData.raw).length, icon: <Activity className="w-4 h-4 text-primary" /> },
                    ].map(stat => (
                      <Card key={stat.label} className="bg-white/5 border-primary/10 p-6 space-y-2 group hover:bg-white/10 transition-colors">
                         <div className="flex items-center gap-2">
                            {stat.icon}
                            <span className="text-[10px] uppercase font-mono text-foreground/40 tracking-widest">{stat.label}</span>
                         </div>
                         <div className="text-3xl font-mono font-black text-foreground italic">{stat.val}</div>
                      </Card>
                    ))}
                 </div>
              </div>
                    ...
                    {auditData.results.map((res, idx) => (
                      ...
                      >
                         <Card className={cn(
                            ...
                            res.status === "CRITICAL" ? "border-red-500/30" : "border-yellow-500/30"
                         )}>
                            <div className="p-5 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                               <div className="space-y-1 flex-1">
                                  <div className="flex items-center gap-3">
                                     {res.status === "SECURE" ? <ShieldCheck className="w-5 h-5 text-primary" /> : <ShieldAlert className="w-5 h-5 text-red-500" />}
                                     <span className="font-mono text-lg font-black italic text-foreground">{res.key}</span>
                                     <Badge className={cn(
                                       "text-[9px] uppercase font-mono italic h-5 border-none",
                                       res.status === "SECURE" ? "bg-primary text-primary-foreground shadow-[0_0_10px_rgba(34,197,94,0.3)]" : 
                                       res.status === "CRITICAL" ? "bg-red-500 text-white" : "bg-yellow-500 text-black"
                                     )}>
                                        {res.status}
                                     </Badge>
                                  </div>
                                  <p className="text-xs text-muted-foreground font-mono leading-relaxed">{res.desc}</p>
                               </div>
              ...
              <Card className="bg-linear-to-br from-primary/20 via-primary/5 to-transparent border-primary/20 p-8 text-center relative overflow-hidden group">
                 ...
                 <div className="relative z-10 max-w-xl mx-auto space-y-6">
                    <div className="space-y-2">
                       <h3 className="text-2xl font-black font-mono uppercase italic tracking-tighter">Initialize Domain Watcher</h3>
                       <p className="text-xs text-muted-foreground font-mono uppercase tracking-widest leading-relaxed">Continuous DNS monitoring. Get alerted the microsecond your MX/SPF/DMARC records are tampered with or expire.</p>
                    </div>
                    <div className="flex flex-col sm:flex-row gap-4 justify-center">
                       ...
                       <Button 
                         ...
                         onClick={() => {
                            const report = `PULSEGUARD DNS INTEGRITY REPORT\n` +
