 ...
 *
 * The function retrieves the user's session and checks for authorization. It then fetches the notification channel configuration from the database. Depending on the channel type (EMAIL, DISCORD, SLACK), it sends a test notification using the appropriate method. If the channel type is not implemented, it returns an error. The function handles various error scenarios and logs any failures during the notification process.
 *
