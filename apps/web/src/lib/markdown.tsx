...
export function MarkdownRenderer({ content }: { content: string }) {
  ...
  const flushList = (keyPrefix: number) => {
    if (listBuffer.length > 0) {
      elements.push(
        <ul
          key={`ul-${keyPrefix}`}
          className="my-4 space-y-2 list-disc list-inside text-muted-foreground"
        >
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith("```")) {
      if (inCodeBlock) {
        elements.push(
          <div
            key={`code-${i}`}
            className="my-6 rounded-lg bg-black/80 border border-border p-4 font-mono text-xs overflow-x-auto text-zinc-200"
          >
            <pre>{codeBuffer.join("\n")}</pre>
          </div>,
        );
    ...
    if (line.startsWith("# ")) {
      elements.push(
        <h1 key={`h1-${i}`} className="text-2xl font-bold tracking-tight text-foreground mt-8 mb-4">
          {renderInline(line.slice(2))}
        </h1>,
      );
    } else if (line.startsWith("## ")) {
      elements.push(
        <h2 key={`h2-${i}`} className="text-xl font-bold tracking-tight text-foreground mt-6 mb-3">
