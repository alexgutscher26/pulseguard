 ...
 * user.
 */
export const probeRegisterRoute: RouteHandler = withErrorHandling(async ({ request, env }, url) => {
  if (url.pathname !== "/api/probes/register" || request.method !== "POST") return null;

  const { verifySession } = await import("../lib/auth");
  const session = await verifySession(request, env);
  if (!session?.userId) throw new AppError(401, "Unauthorized");

  const { name, platform, region, heartbeatInterval } = await requireJsonBody(request);
 ...
 */
export const probePollRoute: RouteHandler = withErrorHandling(async ({ request, env }, url) => {
  if (url.pathname !== "/api/probes/poll" || request.method !== "POST") return null;
 ...
 */
export const probeResultRoute: RouteHandler = withErrorHandling(async ({ request, env }, url) => {
  if (url.pathname !== "/api/probes/result" || request.method !== "POST") return null;
...
export const probeHeartbeatRoute: RouteHandler = withErrorHandling(
  async ({ request, env }, url) => {
    if (url.pathname !== "/api/probes/heartbeat" || request.method !== "POST") return null;
