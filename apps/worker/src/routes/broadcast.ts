 ...
 * durable object.
 */
export const broadcastRoute: RouteHandler = withErrorHandling(async ({ env, request }, url) => {
  if (url.pathname !== "/api/broadcast" || request.method !== "POST") return null;

  const { monitorId, event } = await requireJsonBody(request);
  if (!monitorId || !event) throw new AppError(400, "Missing monitorId or event");
