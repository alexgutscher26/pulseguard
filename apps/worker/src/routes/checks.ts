 ...
 */
export const securityHeadersRoute: RouteHandler = withErrorHandling(
  async ({ request }, url) => {
    ...
    const { checkSecurityHeaders } =
      await import("../services/security-headers");
 ...
 */
export const sslCheckRoute: RouteHandler = withErrorHandling(
  async ({ request }, url) => {
 ...
 */
export const portCheckRoute: RouteHandler = withErrorHandling(
  async ({ request }, url) => {
 ...
 */
export const dnsWatchdogRoute: RouteHandler = withErrorHandling(
  async ({ request }, url) => {
 ...
 */
export const domainExpirationRoute: RouteHandler = withErrorHandling(
  async ({ request }, url) => {
 ...
 */
export const mcpCheckRoute: RouteHandler = withErrorHandling(
  async ({ request }, url) => {
 ...
 */
export const graphqlCheckRoute: RouteHandler = withErrorHandling(
  async ({ request }, url) => {
 ...
 */
export const websocketCheckRoute: RouteHandler = withErrorHandling(
  async ({ request }, url) => {
    ...
    const results = await checkWebSocket(
      targetUrl,
      listenSeconds || 5,
      assertion,
    );
 ...
 */
export const databaseCheckRoute: RouteHandler = withErrorHandling(
  async ({ request }, url) => {
    if (url.pathname !== "/api/database-check" || request.method !== "POST")
      return null;

    const { connectionUrl, query, expectation } =
      await requireJsonBody(request);
    if (!connectionUrl)
      throw new AppError(400, "Missing 'connectionUrl' body param");
 ...
 */
export const bgpCheckRoute: RouteHandler = withErrorHandling(
  async ({ request }, url) => {
 ...
 */
export const globalLatencyRoute: RouteHandler = withErrorHandling(
  async ({ request }, url) => {
