...
export const triggerCmd = new Command("trigger")
  ...
  .action(async (id, opts) => {
    ...
    try {
      ...
      const result = await api.post<TriggerResult>(
      ...
      if (result.httpStatus)
        console.log(chalk.dim(`  HTTP     : ${result.httpStatus}`));
      if (result.errorReason)
        console.log(chalk.red(`  Error    : ${result.errorReason}`));
      console.log(
        chalk.dim(
      ...
      if (err instanceof ApiError) {
        ...
        if (err.status === 422) {
          console.error(
            chalk.dim("Only HTTP monitors support instant trigger"),
          );
        }
      }
      process.exit(1);
