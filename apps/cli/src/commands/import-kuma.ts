...
export function parseKumaExport(
  ...
  return rawList.map((m: any): ParsedKumaMonitor => {
    ...
    // Map Uptime Kuma types to PulseGuard MonitorType
    switch (rawType) {
      case "http":
      case "keyword":
      case "json-query":
...
importCmd
  ...
  .option("--dry-run", "Preview parsed monitors without creating them in PulseGuard")
  ...
  .action(async (filePath: string, opts) => {
    ...
    console.log(chalk.bold("\n📦 PulseGuard Uptime Kuma Importer\n"));
    ...
    if (opts.dryRun) {
      console.log(
        chalk.yellow("✨ Dry run complete. No changes made to your PulseGuard workspace."),
      );
    ...
    const spinner = ora("Connecting to PulseGuard API…").start();
    ...
    console.log(
      `\nAll monitors are now live on PulseGuard's multi-region edge consensus network! 🎉\n`,
    );
  });
