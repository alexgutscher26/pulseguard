...
function getMockData(): CommitData {
  return {
    message: "feat: implemented Stripe advanced billing subscription mapping to netrunner and hobbyist tiers",
    files: [
      "apps/web/components/billing-form.tsx",
...
function categorizeChanges(files: string[]): { category: string; emoji: string } {
  ...
  for (const file of files) {
    if (file.includes("apps/web") || file.includes("components") || file.includes("styles") || file.endsWith(".tsx") || file.endsWith(".css")) {
      frontendCount++;
    } else if (file.includes("prisma") || file.includes("db") || file.includes("schema") || file.endsWith(".sql")) {
      dbCount++;
    } else if (file.includes("apps/api") || file.includes("packages") || file.includes("server") || file.endsWith(".ts")) {
      backendCount++;
    } else {
      configCount++;
    }
...
function cleanCommitMessage(msg: string): string {
  ...
  firstLine = firstLine.replace(/^(feat|fix|docs|style|refactor|test|chore|ci|perf)(\(.+?\))?!?: /, "");
...
function main() {
  ...
  if (copied) {
    ...
    console.log("⚠️ Could not automatically copy to clipboard (platform not supported or utility missing).");
  }
  console.log("=========================================\n");
}
